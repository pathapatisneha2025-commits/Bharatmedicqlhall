  ...
  const verifyCurrentLocation = async () => {
    try {
      ...
      if (distance <= ALLOWED_RADIUS_METERS) {
        ...
        showAlert('Location Error', `Outside allowed radius (${Math.round(distance)}m)`);
      }
    } catch (err) {
  ...
  const endBreak = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        showAlert('✅ Success', 'Break ended successfully!');
      } else {
        showAlert('Error', data.message || 'Failed to end break.');
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <View style={styles.centered}>
        ...
        <Text style={styles.loadingText}>Syncing GPS data...</Text>
      </View>
    );
  }
    ...
    <View style={styles.mainContainer}>
      ...
      <ScrollView style={styles.leftColumn} contentContainerStyle={styles.leftContent}>
        ...
        <View style={styles.brandRow}>
          <div style={styles.logoBadge}><Text style={styles.logoBadgeText}>BM</Text></div>
          <Text style={styles.brandNameText}>Bharat Medical Hall</Text>
        </View>

        <Text style={styles.loginTitle}>End Break</Text>
        <Text style={styles.loginSub}>Resume your duties by completing the verification</Text>
        ...
        <TouchableOpacity style={styles.scanButton} onPress={() => setShowCamera(true)}>
          ...
          <Text style={styles.scanButtonText}>Re-scan Identity</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity 
          ...
          onPress={endBreak}
        >
          <Text style={styles.submitButtonText}>Resume Work</Text>
          <Ionicons name="play-circle-outline" size={20} color="#fff" style={{marginLeft: 10}} />
        </TouchableOpacity>
        ...
        <View style={styles.rightColumn}>
          <View style={styles.brandContent}>
            ...
            <Text style={styles.welcomeSub}>
              Closing your break ensures your active work hours are accurately tracked for today's shift.
            </Text>
          </View>
        </View>
      )}
...
export default SubadminBreakOutScreen;
...
const styles = StyleSheet.create({
  ...
  submitButton: { flexDirection: 'row', backgroundColor: '#0ea5e9', padding: 18, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginTop: 10 },
