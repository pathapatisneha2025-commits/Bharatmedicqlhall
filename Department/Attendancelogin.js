...
const SubadminAttendanceScreen = () => {
    ...
    (async () => {
      try {
        ...
        if (distance <= ALLOWED_RADIUS_METERS) {
          ...
          showAlert('Location Error', `Outside allowed radius.`);
        }
      } catch (error) {
  ...
  const verifyFaceAPI = async (uri) => {
    try {
      ...
      if (data.success && data.faceVerified) {
        ...
        showAlert('✅ Face Verified', data.message);
      } else {
  ...
  const handleSubmit = async () => {
    if (!locationVerified || !faceVerified) {
      showAlert('Error', 'Ensure location and face are verified.');
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) setMessage('✅ Attendance marked successfully!');
    ...
    <View style={styles.mainContainer}>
      ...
      <ScrollView style={styles.leftColumn} contentContainerStyle={styles.leftContent}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backRole}>
          ...
          <Text style={styles.backRoleText}>Back to role selection</Text>
        </TouchableOpacity>
        ...
        <Text style={styles.loginTitle}>Attendance Portal</Text>
        <Text style={styles.loginSub}>Verify your identity and location to check-in</Text>
        ...
        <View style={styles.statusGrid}>
          ...
          <View style={styles.statusCard}>
            ...
            <Text style={styles.cardLabel}>Face Identity</Text>
            <Text style={[styles.cardStatus, { color: faceVerified ? '#22c55e' : '#ef4444' }]}>
              {faceLoading ? '...' : faceVerified ? 'Verified' : 'Pending'}
            </Text>
          </View>
        </View>
        ...
        <TouchableOpacity style={styles.scanButton} onPress={() => setShowCamera(true)}>
          ...
          <Text style={styles.scanButtonText}>Open Front Camera</Text>
        </TouchableOpacity>
      ...
      {/* Right Column - Branding Section (Hidden on small mobile) */}
      {isLargeScreen && (
...
const styles = StyleSheet.create({
  ...
  brandContent: { width: '70%', alignItems: 'center' },
