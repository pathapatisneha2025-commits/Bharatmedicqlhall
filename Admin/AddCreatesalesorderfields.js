...
import { Trash2, PlusCircle, LayoutGrid, CheckCircle2, XCircle } from "lucide-react";

const BASE_URL = "https://hospitaldatabasemanagement.onrender.com";

export default function AdminSalesOrderFields() {
  ...
  const fetchFields = async () => {
    try {
      const res = await fetch(`${BASE_URL}/CreateSalesorderfields/all`);
...
const handleAdd = async () => {
  ...
  const payload = {
    field_key: "custom_" + Date.now(), // unique key
    label: form.field_name,
    type: form.field_type,
    required: form.is_required,
    options: [], // optional for now
  };

  try {
    const res = await fetch(`${BASE_URL}/CreateSalesorderfields/add`, {
      method: "POST",
  ...
  const handleDelete = async (id) => {
    await fetch(`${BASE_URL}/CreateSalesorderfields/delete/${id}`, {
      method: "DELETE",
    });
    fetchFields();
  };
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.sidebar}>
        <View style={styles.headerContainer}>
          ...
          <Text style={styles.title}>Sales Order Field Settings</Text>
          <Text style={styles.subtitle}>Configure dynamic input fields for sales orders.</Text>
        </View>
      ...
      <ScrollView style={styles.mainContent} contentContainerStyle={styles.scrollContent}>
        <View style={styles.tableCard}>
          ...
          {loading ? (
            ...
            <Text style={styles.emptyText}>No sales order fields defined yet.</Text>
          ) : (
            fields.map((field) => (
              <View key={field.id} style={styles.tableRow}>
                <Text style={[styles.cellId, { flex: 0.5 }]}>#{field.id}</Text>
                <Text style={[styles.cellName, { flex: 2 }]}>{field.label}</Text>
                <View style={{ flex: 1 }}>
